...
fun convertLongitude(value: Long) =
    (if (value > 1800000000) value - 4294967296 else value) / 10.0.pow(7.0)

fun convertLatitude(value: Long) =
    (if (value > 900000000) value - 4294967296 else value) / 10.0.pow(7.0)

fun distance(lon1: Double, lat1: Double, lon2: Double, lat2: Double, unit: Char): Double {
    val theta = lon1 - lon2
    var dist =
        sin(lat1.deg2rad()) * sin(lat2.deg2rad()) + cos(lat1.deg2rad()) * cos(lat2.deg2rad()) * cos(
            theta.deg2rad()
        )
...
fun parseConsoleDate(str: String?): ZonedDateTime? {
    return str
